...
import java.sql.*;
import java.util.*;
public class Low_level_language implements ActionListener{
	private JFrame f=new JFrame("Low_level_language");
	...
	public void actionPerformed(ActionEvent ae){
		...
		if((s).equals("Insert")){
			try{
				...
				stmt.executeUpdate("insert into Low_level_languagevalues('"+data_type+"','"+syntax+"','"+description+"')");*/
				String sql = "select * from Low_level_language where data_type='" + t1.getText() + "'";
			...
			try{
				...
				stmt.executeUpdate("Update Low_level_language set data_type='"+data_type+"' where syntax='"+syntax+"' and description='"+description+"'");
				con.close();
			}
			catch(Exception e){
		...
		/*else if((s).equals("Delete")){
			try{
				...
				stmt.executeUpdate("delete from Low_level_language where p_id="+p_id+"");
			    con.close();
			}
			catch(Exception e){
				System.out.println(e);
